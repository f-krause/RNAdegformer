# 2. Supervised learning: Train on ground truth labels
...
def train_fold():
    ...
    dataset = RNADataset(train_seqs, train_labels, train_ids, train_ew, opts.data_path, add_bpps=opts.add_bpps,
                         add_loop_struc=opts.add_loop_struc)
    val_dataset = RNADataset(val_seqs, val_labels, val_ids, val_ew, opts.data_path, training=False,
    ...
    # Checkpointing
    mkdir(opts.project_path, "weights_train")
    checkpoints_folder = os.path.join(opts.project_path, 'weights_train/checkpoints_fold{}'.format(opts.fold))

    # Logging
    mkdir(opts.project_path, "logs_train")
    logs_path = os.path.join(opts.project_path, "logs_train")

    # Store CLI arguments as json for documentation
    with open(os.path.join(logs_path, "train_config.json"), 'wt') as f:
    ...
    weights_pretrained_best = os.path.join(opts.project_path, "weights_pretrain_best", f"fold0top1.ckpt")
    model.load_state_dict(torch.load(weights_pretrained_best))
    ...
    print("Starting training:", time.strftime("%H:%M:%S", time.localtime()))
    ...
    logs_folder = "logs_train"

    print("Training finished:", time.strftime("%H:%M:%S", time.localtime()))
    ...
    get_best_weights_from_fold(opts, logs_folder=logs_folder, weights_folder="weights_train")
